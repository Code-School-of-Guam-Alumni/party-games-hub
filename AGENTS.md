...
3. Read `docs/WORKSTREAMS.md`.
4. Read the GitHub issue assigned to the current branch.
5. Run `git status` and confirm the branch is not `main`.

## Scope rules

- Work on one bounded issue at a time.
- Do not add authentication, payments, real-time multiplayer, room codes, WebSockets, or native/mobile code unless an approved issue explicitly changes MVP scope.
- Do not add or upgrade dependencies without explaining why in the PR.
- Do not change both `api/` and `web/` unless the issue is explicitly a full-stack vertical slice.
